<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="支部名称" prop="zhibumingcheng">
          <el-select :disabled="ro.zhibumingcheng" v-model="ruleForm.zhibumingcheng" placeholder="请选择支部名称">
            <el-option
                v-for="(item,index) in zhibumingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      this.$http({
		...
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/dangzhibu/zhibumingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zhibumingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhibuweiyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhibuweiyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
