...
export default {
  data() {
	...
	shhf : false,
      ...
      rules: {
          shiwuzhuti: [
                { required: true, message: '事务主题不能为空', trigger: 'blur' },
          ],
          shenqingyuanyou: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shiwushenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shiwushenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
